...
class Demo(QWidget):
    def __init__(self):
        ...
        self.bigger_btn = QPushButton('Zoom out', self)         # 4
        self.smaller_btn = QPushButton('Zoom in', self)
    ...
    def bigger_func(self):
        self.label.resize(round(self.label.width()*1.2), round(self.label.height()*1.2))
        self.scrollbar.setMaximum(self.scroll_area.horizontalScrollBar().maximum())
        print(round(self.label.width()*1.2))

    def smaller_func(self):
        self.label.resize(round(self.label.width() * 0.8), round(self.label.height() * 0.8))
        self.scrollbar.setMaximum(self.scroll_area.horizontalScrollBar().maximum())
